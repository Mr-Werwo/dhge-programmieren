// Shellsort-Funktion mit Pointern
// (= Sortieren durch Einfügen mit kleiner werdenden Abständen)
//
// Aufruf: shellsort-ptr anzahl
//
// Klaus Kusche, 2021
...
void sort(int arr[], int anzahl)
{
  ...
  for (int j = 0; j < sizeof(schrittweite)/sizeof(schrittweite[0]); ++j) {
    ...
    for (int *pos = arr + abst; pos < arr + anzahl; ++pos) {
      ...
      for (int *p = pos; (p >= arr + abst) && kleiner(p, p - abst); p -= abst) {
        tausche(p, p - abst);
      }
    }
  }
