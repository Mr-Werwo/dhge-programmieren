// Insertionsort-Funktion mit Pointern (Sortieren durch Einfügen)
//
// Aufruf: inssort-ptr anzahl
//
// Klaus Kusche, 2021
...
{
  // Für jede Position pos von der zweiten bis zur letzten ...
  for (int *pos = arr + 1; pos < arr + anzahl; ++pos) {
    // ... vergleiche und vertausche den Wert von der Stelle pos
    // der Reihe nach (von rechts nach links) immer wieder mit dem Wert davor, 
    // bis der Wert von pos an die richtige Stelle nach links gerückt ist
    // (entweder nach ganz links oder hinter einen kleineren Vorgänger)
    for (int *p = pos; (p > arr) && kleiner(p, p - 1); --p) {
      tausche(p, p - 1);
    }
  }
}
